...
{
    [Fact]
    public void ExposesTheStableInstrumentsWithoutHighCardinalityTags()
    {
        ...
        metrics.CallbackAttempts.Add(1);
        metrics.Attempts.Add(1);
        metrics.QueueDepth.Add(1);

        Assert.Equal(
            ["callback.attempts", "deliveries.failed", "deliveries.recovered", "deliveries.sent", "delivery.attempts", "notifications.accepted", "queue.depth"],
