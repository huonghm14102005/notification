...
public static class ApiKeyEndpoints
{
    ...
    private static async Task<IResult> CreateAsync(CreateApiKeyRequest request, IValidator<CreateApiKeyRequest> validator, ApiKeyHandlers handler, ClaimsPrincipal principal, HttpContext context, CancellationToken ct)
    {
        ...
        try
        {
            var created = await handler.CreateAsync(tenantId, adminId, request.ProducerName, ct); context.Response.Headers.CacheControl = "no-store";
    ...
    private static async Task<IResult> ListAsync(int? limit, string? cursor, ApiKeyHandlers handler, ClaimsPrincipal principal, CancellationToken ct)
    {
        ...
        try { var page = await handler.ListAsync(tenantId, take, cursor, ct); return Results.Ok(new { items = page.Items, nextCursor = page.NextCursor }); }
        catch (ApiKeyOperationException exception) { return Error(exception.Code); }
    }

    private static async Task<IResult> RevokeAsync(Guid id, ApiKeyHandlers handler, ClaimsPrincipal principal, CancellationToken ct)
    {
        if (!TryIdentity(principal, out var tenantId, out _)) return Results.Unauthorized();
        try { await handler.RevokeAsync(tenantId, id, ct); return Results.NoContent(); }
        catch (ApiKeyOperationException exception) { return Error(exception.Code); }
    }
