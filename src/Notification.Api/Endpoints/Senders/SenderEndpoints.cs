...
public static class SenderEndpoints
{
    ...
    private static async Task<IResult> TestAsync(Guid id, JsonElement body, IValidator<SendTestEmailRequest> validator, SendTestEmailHandler handler, ClaimsPrincipal p, CancellationToken ct)
    {
        ...
        catch (EmailSendException x) { return x.Timeout ? Results.Json(new { error = "SMTP test timed out", code = "SMTP_TEST_TIMEOUT", statusCode = 504 }, statusCode: 504) : Results.Json(new { error = "SMTP test failed", code = "SMTP_TEST_FAILED", statusCode = 502, reason = x.Reason }, statusCode: 502); }
    }
