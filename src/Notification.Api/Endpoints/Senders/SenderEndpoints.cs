...
public static class SenderEndpoints
{
    public static IEndpointRouteBuilder MapSenderEndpoints(this IEndpointRouteBuilder e)
    {
        ...
        e.MapDelete("/v1/senders/{id:guid}", DisableAsync).RequireAuthorization("Admin").RequireRateLimiting("sender-mutation"); return e;
    }
    ...
    private static IResult Error(string code) => code switch { "NOT_FOUND" => Results.NotFound(new { error = "Not found", code, statusCode = 404 }), "SENDER_KEY_EXISTS" or "SENDER_DISABLED" => Results.Conflict(new { error = "Sender conflict", code, statusCode = 409 }), _ => Results.BadRequest(new { error = "Validation failed", code, statusCode = 400 }) };
}
