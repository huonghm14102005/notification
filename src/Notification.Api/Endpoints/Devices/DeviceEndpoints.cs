...
public static class DeviceEndpoints
{
    ...
    {
        endpoints.MapPost("/v1/devices", CreateAsync).RequireAuthorization("Admin").RequireRateLimiting("device-create");
        endpoints.MapGet("/v1/devices", ListAsync).RequireAuthorization("Admin");
        endpoints.MapGet("/v1/devices/{id:guid}", GetAsync).RequireAuthorization("Admin");
        endpoints.MapPatch("/v1/devices/{id:guid}", RenameAsync).RequireAuthorization("Admin");
        endpoints.MapPost("/v1/devices/{id:guid}/disable", DisableAsync).RequireAuthorization("Admin");
        endpoints.MapPut("/v1/devices/{id:guid}/callback", ConfigureCallbackAsync).RequireAuthorization("Admin");
        endpoints.MapDelete("/v1/devices/{id:guid}/callback", ClearCallbackAsync).RequireAuthorization("Admin");
        endpoints.MapPost("/v1/devices/{deviceId:guid}/api-keys", CreateKeyAsync).RequireAuthorization("Admin").RequireRateLimiting("api-key-create");
        endpoints.MapGet("/v1/devices/{deviceId:guid}/api-keys", ListKeysAsync).RequireAuthorization("Admin");
        endpoints.MapDelete("/v1/devices/{deviceId:guid}/api-keys/{keyId:guid}", RevokeKeyAsync).RequireAuthorization("Admin");
        return endpoints;
    }
    ...
    { "NOT_FOUND" => Results.NotFound(new { error = "Not found", code, statusCode = 404 }), "FORBIDDEN" => Results.Json(new { error = "Forbidden", code, statusCode = 403 }, statusCode: 403), "DEVICE_DISABLED" or "DEVICE_API_KEY_LIMIT_REACHED" or "API_KEY_LIMIT_REACHED" => Results.Conflict(new { error = "Conflict", code, statusCode = 409 }), _ => Results.BadRequest(new { error = "Validation failed", code, statusCode = 400 }) };
}
