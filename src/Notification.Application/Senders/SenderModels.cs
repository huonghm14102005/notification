...
public sealed record ResolvedSender(Guid Id, Guid TenantId, string Key, string Channel, string Host, int Port, bool Secure, string Username, byte[] PasswordEncrypted, string FromEmail, string FromName, string Status = "active");
public sealed record SendTestEmailResult(bool Sent, Guid SenderId, string RecipientEmail, DateTimeOffset VerifiedAt);
