...
public sealed class ApiKeyHandlers(IIdentityRepository repository, IDeviceRepository devices, IApiKeySecretService secrets, IClock clock)
{
    public async Task<CreatedApiKey> CreateAsync(Guid tenantId, Guid adminId, string producerName, CancellationToken ct)
    {
        ...
        var device = await devices.GetOrCreateLegacyAsync(tenantId, adminId, producerName, now, ct);
        var entity = new ApiKey(Guid.NewGuid(), tenantId, adminId, device.Id, producerName.Trim(), secret.Prefix, secret.Hash, now);
