    ...
    ISecretCipher cipher, IClock clock, NotificationMetrics metrics)
{
    public async Task<AcceptNotificationResult> HandleAsync(Guid tenantId, Guid apiKeyId, AcceptNotificationCommand command, CancellationToken ct)
    {
        ...
        var notification = new OutboundNotification(id, tenantId, apiKeyId, sender.Id, command.Recipient.Email,
            command.Recipient.Ref, cipher.Encrypt(command.Subject, tenantId, id), cipher.Encrypt(command.Body, tenantId, id), now);
        try { await repository.AddAsync(notification, ct); }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
        catch { throw new NotificationOperationException("SERVICE_UNAVAILABLE"); }
        metrics.Accepted.Add(1);
        return new(1, [new(id, command.Recipient.Email, command.Recipient.Ref)]);
    }
}
