...
public sealed class DeliveryAttemptConfiguration : IEntityTypeConfiguration<DeliveryAttempt>
{
    public void Configure(EntityTypeBuilder<DeliveryAttempt> b)
    {
        ...
        b.Property(x => x.DeliveryId).HasColumnName("delivery_id"); b.Property(x => x.SenderId).HasColumnName("sender_id");
        ...
        b.HasIndex(x => new { x.DeliveryId, x.AttemptNo }).IsUnique().HasDatabaseName("ux_delivery_attempts_delivery_no");
        b.HasIndex(x => new { x.TenantId, x.CreatedAt }).HasDatabaseName("ix_delivery_attempts_tenant_created");
        b.HasIndex(x => new { x.TenantId, x.DeliveryId }).HasDatabaseName("ix_delivery_attempts_tenant_delivery");
        b.HasOne(x => x.Tenant).WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict);
        b.HasOne(x => x.Delivery).WithMany().HasForeignKey(x => x.DeliveryId).OnDelete(DeleteBehavior.Restrict);
