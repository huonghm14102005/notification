...
public sealed class AdminConfiguration : IEntityTypeConfiguration<Admin>
{
    ...
    {
        builder.ToTable("admins", table =>
        {
            table.HasCheckConstraint("ck_admins_role", "role IN ('owner','member')");
            table.HasCheckConstraint("ck_admins_status", "status IN ('active','disabled')");
            table.HasCheckConstraint("ck_admins_disabled", "(status = 'active' AND disabled_at IS NULL) OR (status = 'disabled' AND disabled_at IS NOT NULL)");
